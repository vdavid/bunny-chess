...
export default function Timer({ playerIndex, isLight, isLightTurn, targetMs, elapsedMs }: Props) {
    ...
    return <div style={{ textAlign: 'center', backgroundColor: colors[playerColors[playerIndex]][500] }}
        ...
        {isLightTurn === isLight ? <strong>{playerNames[playerIndex]}</strong> : playerNames[playerIndex]}:
        <big>{minutes}</big>:{seconds}<small>.{milliseconds}</small>
    </div>
}
